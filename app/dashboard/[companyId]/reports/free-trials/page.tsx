...
export default function ReportsLeadsPage() {
  ...
  useEffect(() => {
    if (data && group == 'month') {
      setAMonthAgo(data[data.length - 2]?.trials || 0);
      setTwoMonthsAgo(data[data.length - 3]?.trials || 0);
      setThreeMonthsAgo(data[data.length - 4]?.trials || 0);
      setCurrentValue(data[data.length - 1]?.trials || 0);
    }
  }, [group, data])
