...
export default function ReportsMRRPage() {
  ...
  useEffect(() => {
    if (data && group == 'month') {
      setAMonthAgo(data[data.length - 2]?.churnedMRR || 0);
      setTwoMonthsAgo(data[data.length - 3]?.churnedMRR || 0);
      setThreeMonthsAgo(data[data.length - 4]?.churnedMRR || 0);
      setCurrentValue(data[data.length - 1]?.churnedMRR || 0);
    }
  }, [group, data])
      ...
      <CustomerTitle
        ...
        icon={
          <BsFillQuestionCircleFill
            ...
            title='Track MRR lost from cancellations.'
          />
        }
