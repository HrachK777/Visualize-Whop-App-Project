...
const filterOptions = constants.leadsFilterOptions;
const categories = constants.leadsCategories;
...
export default function ReportsLeadsPage() {
  ...
  const [selectedFilter, setSelectedFilter] = useState('Leads & Conversions');
  const allFilterLabel = 'Leads & Conversions';
  ...
  useEffect(() => {
    if (data && group == 'month') {
      setAMonthAgo(data[data.length - 2]?.activeCustomers || 0);
      setTwoMonthsAgo(data[data.length - 3]?.activeCustomers || 0);
      setThreeMonthsAgo(data[data.length - 4]?.activeCustomers || 0);
      setCurrentValue(data[data.length - 1]?.activeCustomers || 0);
    }
  }, [group, data])
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fc] px-10 py-4 space-y-6">
      {/* Header */}
      <CustomerTitle
        title="Leads (Active Trials)"
        icon={
          <BsFillQuestionCircleFill
            className="h-5 w-5 text-gray-500 ml-2"
            title='People currently testing your product before committing to pay.'
          />
        }
