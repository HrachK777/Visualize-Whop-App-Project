...
export default function ReportsMRRPage() {
  ...
  useEffect(() => {
    if (data && group == 'month') {
      setAMonthAgo(data[data.length - 2]?.contractionMRR || 0);
      setTwoMonthsAgo(data[data.length - 3]?.contractionMRR || 0);
      setThreeMonthsAgo(data[data.length - 4]?.contractionMRR || 0);
      setCurrentValue(data[data.length - 1]?.contractionMRR || 0);
    }
  }, [group, data])
      ...
      <CustomerTitle
        ...
        icon={
          <BsFillQuestionCircleFill
            ...
            title='Track MRR loss from downgrades.'
          />
        }
