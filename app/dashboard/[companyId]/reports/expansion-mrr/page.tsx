...
export default function ReportsMRRPage() {
  ...
  useEffect(() => {
    if (data && group == 'month') {
      setAMonthAgo(data[data.length - 2]?.expansionMRR || 0);
      setTwoMonthsAgo(data[data.length - 3]?.expansionMRR || 0);
      setThreeMonthsAgo(data[data.length - 4]?.expansionMRR || 0);
      setCurrentValue(data[data.length - 1]?.expansionMRR || 0);
    }
  }, [group, data])
      ...
      <CustomerTitle
        ...
        icon={
          <BsFillQuestionCircleFill
            ...
            title='Track MRR growth from existing customers.'
          />
        }
