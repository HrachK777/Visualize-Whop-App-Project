...
const filterOptions = constants.churnFilterOptions;
const categories = constants.churnCategories;
...
export default function ReportsChurnPage() {
  ...
  const [selectedFilter, setSelectedFilter] = useState('Churn & Retention');
  const allFilterLabel = 'Churn & Retention';

  // ✅ Fetch analytics data using your custom hook
  const { data, loading, error } = useAnalyticsData(group, "LTV");
  ...
  useEffect(() => {
    if (data && group == 'month') {
      setAMonthAgo(data[data.length - 2]?.ltv || 0);
      setTwoMonthsAgo(data[data.length - 3]?.ltv || 0);
      setThreeMonthsAgo(data[data.length - 4]?.ltv || 0);
      setCurrentValue(data[data.length - 1]?.ltv || 0);
    }
  }, [group, data])
      ...
      <CustomerTitle
        ...
        icon={
          <BsFillQuestionCircleFill
            ...
            title='The total revenue you expect from a customer over their entire relationship.'
          />
        }
