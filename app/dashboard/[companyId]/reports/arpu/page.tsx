...
export default function ReportsChurnPage() {
  ...
  useEffect(() => {
    if (data && group == 'month') {
      setAMonthAgo(data[data.length - 2]?.arpu || 0);
      setTwoMonthsAgo(data[data.length - 3]?.arpu || 0);
      setThreeMonthsAgo(data[data.length - 4]?.arpu || 0);
      setCurrentValue(data[data.length - 1]?.arpu || 0);
    }
  }, [group, data])
